...
func deployContract(ctx context.Context, compiledContractFileName string) error {
    ...
    envCmd := cmd.NewCmd("/bin/sh", "-c", 
        fmt.Sprintf("soroban deploy --wasm ./%s/target/wasm32-unknown-unknown/release/%s --rpc-url %s --secret-key %s --network-passphrase \"%s\"", 
...
func invokeContractFromCliTool(testConfig *testConfig, functionName string, contractName string, param1 string) (string, error){
    ...
    if (param1 != "") {
        param1Flag = "--arg"
        param1Value = fmt.Sprintf("\"%s\"",param1)
    }

    envCmd := cmd.NewCmd("/bin/sh", "-c", 
                         fmt.Sprintf("soroban invoke --fn \"%s\" %s %s --id %s --rpc-url %s --secret-key %s --network-passphrase \"%s\"",
                             functionName,
                             param1Flag,
                             param1Value,
